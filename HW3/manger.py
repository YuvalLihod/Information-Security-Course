...
def find_f1(k, key, c, oracle):
    ...
    #?
    while True:
        c0 = (c * pow(f1, key.e, key.n)) % key.n
        if not oracle.query(c0.to_bytes(k, byteorder="big")):  # == (m * f1) ** e mod n
            return f1
        f1 *= 2
...
def find_f2(k, key, c, f1, oracle):
    ...
    B = 2 ** (8 * (k - 1))
    #?
    f2 = (divfloor(key.n + B, B)) * divfloor(f1, 2)
    while True:
        c0 = (c * pow(int(f2), key.e, key.n)) % key.n
        if oracle.query(c0.to_bytes(k, byteorder="big")):  # == (m * f2) ** e mod n
            return f2
        f2 += divfloor(f1, 2)
...
def find_m(k, key, c, f2, oracle, verbose=False):
    ...
    while m_max != m_min:
        ...
        c_i = (c * pow(int(f3), key.e, key.n)) % key.n
        if oracle.query(c_i.to_bytes(k, byteorder="big")):  # == (m * f3) ** e mod n
